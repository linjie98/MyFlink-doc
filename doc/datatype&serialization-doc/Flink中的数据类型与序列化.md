...
#### 主要是理解TypeInformation类
该类是所有类型描述符的基类，它揭示了一些基本属性，并且可以生成序列化器，也可以生成类型的比较器，其源码如下(大部分方法有笔者自己的理解)
...
public abstract class TypeInformation<T> implements Serializable {
	 ...
	 */
	//返回map，通过查看emptyMap源码可得知其默认返回一个空的map
	@PublicEvolving
	 ...
	 */
	//判断该类型可否用作key，可以返回true，否则返回false
	@PublicEvolving
...
#### Flink支持的DataType
- 基本类型：All Java primitives and their boxed form, plus void, String, Date, BigDecimal, and BigInteger.
- 基本数组与对象
- 符合类型
    - Flink Tuples(Flink API的一部分)：最多25个字段，不支持空字段
    - Scala case classes
    - Row：具有任意数量的元组并支持空字段
    - POJO：类似于bean模式的类
...
#### POJO类型的规则
满足以下条件，Flink就会将数据类型识别为POJO类（并允许按名称字段引用）
```java
dataSet.join(another).where("name").equalTo("personName")
```

- 该类是public和standalone(独立的)（且应该是静态内部类）
```java
/**
...
```
- 该类有一个公共的无参构造函数
- 该类以及所有超类，所有non-static, non-transient的字段都是公共的以及都是非final常量的，或者有公共的getter和setter方法，该方法遵循getter和setter的Java bean命名约定。

**注意：当用户定义的数据类型无法识别为POJO类型时，必须将其作为GenericType处理并使用Kryo进行序列化。**


#### Flink无法实现序列化的数据类型
Flink无法实现序列化的数据类型，可以交给Avro或者Kryo

如果Kryo无法处理该类型，您可以要求PojoTypeInfo使用Avro序列化POJO
```java
