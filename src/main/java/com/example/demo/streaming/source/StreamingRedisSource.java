 ...
 * by：linjie
 */
public class StreamingRedisSource implements SourceFunction<HashMap<String,String>> {

    private static Logger logger = LoggerFactory.getLogger(StreamingRedisSource.class);
    //设置每次取数据的休眠时间常量
    ...
    public void run(SourceContext<HashMap<String, String>> ctx) throws Exception {
        ...
        while (isRunning){
            try {
                ...
                shops.clear();
                shops = (HashMap<String, String>) jedis.hgetAll("test");
                if (shops.size()>0){
